...
export const SigninPage = () => {
  ...
  const handleSignIn = (e) => {
    ...
    let authData = JSON.parse(localStorage.getItem(CONSTANTS.AUTH_DATA)) || {
      AdminData: [],
      HrData: [],
      UserData: [],
    };
    ...
    if (userRole === CONSTANTS.ADMIN) {
      authData.AdminData.push(newUser);
    } else if (userRole === CONSTANTS.HR) {
      authData.HrData.push(newUser);
    } else if (userRole === CONSTANTS.USER) {
      authData.UserData.push(newUser);
    } else {
      alert("Invalid role selected.");
      return;
    }

    // Store updated authData in localStorage
    localStorage.setItem(CONSTANTS.AUTH_DATA, JSON.stringify(authData));

    // Verify storage (for debugging)
    // console.log("Updated LocalStorage Data:", JSON.parse(localStorage.getItem("authData")));
            ...
            <form noValidate="" className="mb-4" onSubmit={handleSignIn}>
              <div className="grid gap-2">
                ...
                <p className="text-white">
                  ...
                  <Link
                    to="/login"
                    className="ms-2 text-blue-600 font-semibold hover:underline underline-offset-2"
                  >
                    / login
                  </Link>
                </p>
